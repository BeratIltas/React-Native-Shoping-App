                    ...
                    <View style={styles.orderCard}>
                        ...
                        <View style={styles.orderCard2}>
                            <Text style={[{ fontWeight: "bold" }, typography.Body2]}>
                                <Text style={[{ fontWeight: "normal" }, typography.Body2Regular]}>Items: </Text>{item.items.length}
                            </Text>
                            <View style={styles.orderCard2Sub}>
                                <Text style={[{ fontWeight: "bold" }, typography.Body2Medium]}>
                                    <Text style={[{ fontWeight: "normal" }, typography.Body2Regular]}>Quantity: </Text>
                                    {item.items.reduce((sum: number, i: any) => sum + i.quantity, 0)}
                                </Text>
                                <Text style={[{ fontWeight: "bold" }, typography.Body2Medium]}>
                                    <Text style={[{ fontWeight: "normal" }, typography.Body2Regular]}>Total: </Text>${item.total_price}
                                </Text>
                            </View>
                        </View>
                        <View style={styles.orderCard3}>
                            <TouchableOpacity style={styles.detailsButton} onPress={() => navigation.navigate("OrderDetailsScreen", { orderId: item.id })}>
                                <Text style={[styles.detailsButtonText, typography.Body2]}>Details</Text>
                            </TouchableOpacity>
                            <Text
                                style={[{ color: getStatusColor(item.status) }, typography.Body2]}>{item.status.charAt(0).toUpperCase() + item.status.slice(1)}
                            </Text>
                        </View>
                    </View>
                )}
